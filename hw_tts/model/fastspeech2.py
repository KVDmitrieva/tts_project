...
class VarianceAdaptor(nn.Module):
    ...
    def forward(self, x, alpha=1.0, target=None, mel_max_length=None):
        output, duration_predictor_output = self.length_regulator(x, alpha, target, mel_max_length)
        pitch_out = self.pitch_predictor(output)
        energy_out = self.energy_predictor(output)
...
class FastSpeech2(BaseModel):
    ...
    def __init__(self, encoder_params, lr_params, decoder_params, num_mels):
        ...
        self.length_regulator = LengthRegulator(**lr_params, predictor=UniversalPredictor)
